 ...
 * Format: www.quirkchevynh.com/inventory/new-{year}-chevrolet-{model}-{trim}-{drive}-{body}-{vin}
 */
export function generateVehicleUrl(row: InventoryRow): string {
  ...
  if (modelUpper.includes("SILVERADO") || modelUpper.includes("COLORADO")) {
    bodyType = "crew-cab";
  ...
  return `https://www.quirkchevynh.com/inventory/${path}/`;
}
